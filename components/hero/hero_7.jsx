...
import * as PERSONA_CONTRACT from '../../blockchain/PersonaExisNft.blockchain'

const newFileName = "train-image";
...
const Hero_6 = () => {
  ...
  const config = {
    bucketName: "aimodels-cyian",
    dirName: `models/${account}/training_images` /* optional */,
    region: "ap-southeast-1",
    accessKeyId: "AKIAXQVVZIVQ4K3W47NP",
    secretAccessKey: "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",
  };

  const ReactS3Client = new S3(config);

  const handleUploadImageToS3 = async (acceptedFiles) => {
    var results = await Promise.all(
      acceptedFiles.map(async (file, index) => {
        const abc = await ReactS3Client.uploadFile(file, `${index}.png`);
        return abc;
      })
    );
  ...
  async function handleMint() {
    if (window.ethereum) {
      ...
      const contract = new ethers.Contract(
        PERSONA_CONTRACT.ADDRESS,
        PERSONA_CONTRACT.ABI,
        signer
      )

      try {
        const response = await contract.mint()
        console.log("reponse from mint: ", response)
        const tokenId = totalSupply

        // TODO: Build API interaction to notify the backend
        // on the newly minted tokenID
      } catch (error) {
  ...
  async function getTotalSupply() {
    if (window.ethereum) {
      ...
      try {
        ...
        setTotalSupply(response.toNumber() + 1)
      } catch (error) {
        console.log("Error: ", error)
      }
    }
          ...
          <div className="grid h-full items-center gap-4 lg:grid-cols-12">
            <div className="col-span-6 flex h-full flex-col items-center justify-center py-10 lg:items-start lg:py-20">
              ...
              <p className="mb-8 max-w-md text-center text-lg dark:text-jacarta-200 lg:text-left">
                The world of blockchain-based digital identities or "Personas",
                ntersects with the metaverse, which allows players to create
                their own avatars.
              </p>
                  ...
                  <div className="border-2 ">
                    <Dropzone
                      onDrop={(acceptedFiles) =>
                        handleUploadImageToS3(acceptedFiles)
                      }
                    >
                ...
                <div className="absolute -left-[0%] bottom-1/4 inline-block animate-fly rounded-2lg bg-[#EAF7E1] p-6 shadow-2xl sm:left-[0%] md:left-0">
                  <div className="flex gap-4">
                    <span className="inline-flex h-12 w-12 items-center justify-center rounded-full bg-white">
                      ...
                      >
                        <g
                          transform="translate(0.000000,24) scale(0.100000,-0.100000)"
                          fill="#10b981"
                          stroke="none"
                        >
                          <path
                            d="M24 207 c-2 -8 -4 -52 -2 -98 l3 -84 95 0 95 0 0 95 0 95 -93 3 c-71
                          ...
                          />
                          <path
                            d="M130 164 c-11 -12 -10 -18 3 -32 16 -15 18 -15 34 0 13 14 14 20 3
